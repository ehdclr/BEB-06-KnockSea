...
contract KnockSeaMarket {
    ...
    event SaleCanceled(uint256 tokenId, uint256 itemId, address seller);
    ...
    function sell(
        ...
        idMarketItem[itemId] = MarketItem({
            itemId: itemId,
            token: NFTToken({contractAddr: _contractAddr, tokenId: _tokenId}),
        ...
        emit ItemCreated(_tokenId, itemId, msg.sender, _price);
    }

    function saleCancel(address _contractAddr, uint256 _itemId) public {
        require(
            msg.sender == idMarketItem[_itemId].seller,
            "only seller can cancel"
        );
        require(
            ItemStatus.ON_SALE == idMarketItem[_itemId].itemStatus,
            "item is not on sale"
        );
        ...
        emit SaleCanceled(tokenId, _itemId, msg.sender);
    }

    function buy(address _contractAddr, uint256 _itemId) public payable {
